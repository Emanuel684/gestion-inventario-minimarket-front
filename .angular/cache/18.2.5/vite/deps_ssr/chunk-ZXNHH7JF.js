      ...
      if (extended) {
        ...
        const index = rules.findIndex((rule) => {
          if (Array.isArray(rule)) {
            const [from2, to] = rule;
            const afterFrom =
              currentHours >= from2.hours && currentMinutes >= from2.minutes;
            const beforeTo =
...
function findAnchorFromDocument(document2, target) {
  const documentResult =
    document2.getElementById(target) || document2.getElementsByName(target)[0];
  if (documentResult) {
...
var HttpClient = class _HttpClient {
  ...
  request(first, url, options = {}) {
    ...
    const events$ = (0, import_rxjs.of)(req).pipe(
      (0, import_operators.concatMap)((req2) => this.handler.handle(req2)),
    );
    if (first instanceof HttpRequest || options.observe === "events") {
      return events$;
    }
    const res$ = events$.pipe(
      (0, import_operators.filter)((event) => event instanceof HttpResponse),
    );
  ...
  return (req, handler) => {
    ...
    if (contributeToStability) {
      ...
      return chain(req, handler).pipe(
        (0, import_operators.finalize)(() => pendingTasks.remove(taskId)),
      );
    } else {
  ...
  handle(initialRequest) {
    ...
    if (this.contributeToStability) {
      ...
      return this.chain(initialRequest, (downstreamRequest) =>
        ...
        (0, import_operators.finalize)(() => this.pendingTasks.remove(taskId)),
      );
    } else {
      return this.chain(initialRequest, (downstreamRequest) =>
        this.backend.handle(downstreamRequest),
...
var HttpXhrBackend = class _HttpXhrBackend {
  ...
  handle(req) {
    ...
    const source = xhrFactory.ɵloadImpl
      ? (0, import_rxjs.from)(xhrFactory.ɵloadImpl())
...
var EventDelegationPlugin = class _EventDelegationPlugin extends EventManagerPlugin {
  ...
  static {
    this.ɵfac = function EventDelegationPlugin_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _EventDelegationPlugin)(
        ɵɵinject(DOCUMENT),
      );
    };
  }
...
@angular/platform-browser/fesm2022/platform-browser.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-ZXNHH7JF.js.map
